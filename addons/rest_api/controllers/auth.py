...
#       }
# OUT data:
OUT_auth_gettokens_SUCCESS_CODE = 200             # editable
...
#       }
# OUT data:
OUT_auth_refreshtoken_SUCCESS_CODE = 200          # editable
#   Possible ERROR CODES:
...
#       {"refresh_token": "XXXXXXXXXXXXXXXXX"}
# OUT data:
OUT_auth_deletetokens_SUCCESS_CODE = 200          # editable
#   Possible ERROR CODES:
#       400 'no_refresh_token'
...
class ControllerREST(http.Controller):
    ...
    def api_auth_gettokens(self, **kw):
        ...
        token_store.save_all_tokens(request.env, access_token=access_token, expires_in=expires_in,
                                    refresh_token=refresh_token, refresh_expires_in=refresh_expires_in, user_id=uid)
        ...
        resp = werkzeug.wrappers.Response(
            status=OUT_auth_gettokens_SUCCESS_CODE,
    ...
    def api_auth_refreshtoken(self, **kw):
        ...
        # Successful response:
        resp = werkzeug.wrappers.Response(
            status=OUT_auth_refreshtoken_SUCCESS_CODE,
            content_type='application/json; charset=utf-8',
            headers=[('Cache-Control', 'no-store'), ('Pragma', 'no-cache')],
            response=json.dumps({
    ...
    def api_auth_deletetokens(self, **kw):
        ...
        # Successful response:
        return successful_response(OUT_auth_deletetokens_SUCCESS_CODE, {})
